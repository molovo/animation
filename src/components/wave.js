...
const Wave = () => {
  ...
  const transform = useCallback(
    ([x, y, oy]) => [x, oy + 25 * Math.sin(x / 80 + offset), oy],
    [offset],
  )
  ...
  useEffect(() => {
    ...
    warp.current.transform(transform)
    // requestAnimationFrame(() => {
    setOffset((offset) => offset + 0.05)
    // })
  }, [offset])
  ...
  useEffect(() => {
    if (svg.current) {
      ...
      warp.current.interpolate(70)
      warp.current.extrapolate(70)
